...
import { validateBoulder, getBoulderValidations, getUserValidations } from '#services'

export const validateBoulderController = async (req: Request, res: Response) => {
  if (!req.user) {
    res.status(401).json({
      success: false,
...
export const getBoulderValidationsController = async (req: Request, res: Response) => {
...
export const getUserValidationsController = async (req: Request, res: Response) => {
  if (!req.user) {
    res.status(401).json({
      success: false,
      message: 'User not authenticated'
